...
import type { ManualEntry } from '../domain/content'
import ManualInline from './ManualInline.vue'
defineProps<{ entry: ManualEntry }>()
...
<template>
  <section class="content-section">
    ...
    <p class="manual-notice">
      参考区间、临床意义和业务要求未经本软件医学校订。请以检验报告及就诊医院现行规定为准。
    </p>
    ...
    <dl class="manual-fields">
      <div v-for="(field, index) in entry.fields" :key="index">
        <dt>{{ field.label }}</dt>
        <dd>
          <ManualInline v-if="field.value" :text="field.value" />
          ...
          <span v-if="field.inheritedFrom" class="manual-inherited"
            ...
            /><br /><RouterLink
              :to="{
                name: 'item',
                params: { itemId: field.inheritedFrom.itemId },
                query: route.query,
              }"
              >查看同上所指记录</RouterLink
            ></span
