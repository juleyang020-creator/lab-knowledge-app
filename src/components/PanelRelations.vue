...
defineProps<{ knowledge: ItemKnowledge }>()
...
<template>
  ...
  >
    ...
    <p class="knowledge-muted">
      按原表列出的名称或缩写查找同源条目。同名关联仅供查阅，不是医嘱等价确认，不能据此合管采样。
    </p>
    ...
    <div class="member-list">
      <details v-for="(member, index) in knowledge.members" :key="index" class="member-row">
        <summary>
          <span class="member-token"><ManualInline :text="member.label" /></span>
        ...
        <div v-if="!member.candidates.length" class="member-reading">
          ...
          <RouterLink class="text-link" :to="{ name: 'items', query: { q: member.label } }"
            >按原词查找项目</RouterLink
          >
        </div>
        <div v-for="candidate in member.candidates" :key="candidate.id" class="member-reading">
          <RouterLink
            ...
            :to="{ name: 'item', params: { itemId: candidate.id }, query: route.query }"
          ...
          <p>
            <strong>该条目标本：</strong
            ><ManualInline :text="fieldValue(candidate, '标本要求') || '原文未填写'" />
          </p>
          <p>
            <strong>临床意义：</strong
            ><ManualInline :text="fieldValue(candidate, '临床意义') || '原文未填写'" />
          </p>
        </div>
      </details>
    </div>
  ...
  >
    ...
    <div class="related-panels">
      <RouterLink
        ...
        :key="panel.id"
        :to="{ name: 'item', params: { itemId: panel.id }, query: route.query }"
