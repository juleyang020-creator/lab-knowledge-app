...
test('跨文件重复段落修正后，来源重试刷新三文件并保留患者收藏与阅读偏好', async ({ page }) => {
  let server = contentBundle()
  server.patient.articles[0]!.sections[0]!.claims[0]!.id = 'sample.professional'
  const requests = { catalog: 0, professional: 0, patient: 0 }
  ...
  await page.goto('/#/patient/items/sample')
  await expect(page.getByRole('heading', { name: '通俗说明', exact: true })).toBeVisible()
  expect(requests).toEqual({ catalog: 1, patient: 1, professional: 0 })
  await page.getByRole('button', { name: '收藏测试项目', exact: true }).click()
  const preferences = await page.evaluate((key) => {
    localStorage.setItem('unrelated-setting', 'keep')
    return localStorage.getItem(key)
  }, PREFERENCE_KEY)
  expect(JSON.parse(preferences!)).toMatchObject({ savedIds: ['sample'], audience: 'patient' })
  await page.getByRole('link', { name: '来源与进度', exact: true }).click()
  await expect(page.getByRole('alert')).toContainText('资料之间的来源信息不一致')
  await expect(page.getByTestId('claim-count')).toHaveCount(0)
  expect(requests).toEqual({ catalog: 1, patient: 1, professional: 1 })
  ...
  await expect.poll(() => requests).toEqual({ catalog: 2, patient: 2, professional: 2 })
  await expect(page.getByRole('alert')).toContainText('资料之间的来源信息不一致')
  await expect(page.getByTestId('claim-count')).toHaveCount(0)
  ...
  await expect(page.getByTestId('model-count')).toHaveText('3')
  await expect(page.getByTestId('claim-count')).toHaveText('4')
  await expect(page.getByText('刷新后的测试资料 · 测试版', { exact: true })).toBeVisible()
  expect(requests).toEqual({ catalog: 3, patient: 3, professional: 3 })
...
) as { total: number; document: number; model: number }

test('来源进度页显示真实段落统计，明确模型补充未完成替换', async ({ page }) => {
  await page.goto('/#/sources')
  await expect(page.getByRole('heading', { name: '来源与补全进度', exact: true })).toBeVisible()
  await expect(page.getByTestId('document-count')).toHaveText(String(expectedStats.document))
  await expect(page.getByTestId('model-count')).toHaveText(String(expectedStats.model))
  await expect(page.getByTestId('claim-count')).toHaveText(String(expectedStats.total))
